...
app.post('/api/auth/login', {
  config: {
    rateLimit: { max: 10, timeWindow: '15 minute' }, // Stricter for login
  },
...
app.post('/api/terminal/session', async (request: any, reply: any) => {
  ...
  try {
    const { containerId, container } = await getOrCreateContainer(user.id);
...
app.get('/api/files/download', async (request: any, reply: any) => {
  ...
  try {
    const { stream, filename, size } = await getFileStream(user.id, filePath);
...
app.post('/api/files/upload', async (request: any, reply: any) => {
  ...
  const uploadPath = path.join(process.env.STORAGE_ROOT || '/data/users', user.id, filename);

  try {
    const buffer = await data.toBuffer();
    await fs.writeFile(uploadPath, buffer);
    return reply.code(201).send({ success: true, filename });
  } catch (err) {
